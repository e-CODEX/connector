...
public class WebMessagesGrid extends PaginatedGrid<WebMessage> implements AfterNavigationObserver {
    ...
    private final Map<String, Column<WebMessage>> hideableColumns = new HashMap<>();
    ...
    private void addAllColumns() {
        ...
        addHideableColumn(
                WebMessage::getDeliveredToNationalSystem,
                "delivered backend",
                "300px",
                "deiliveredToNationalSystem",
                true,
                false
        );
        addHideableColumn(
                WebMessage::getDeliveredToGateway,
                "delivered gateway",
                "300px",
                "deliveredToGateway",
                true,
    ...
    private void addHideableColumn(
            ValueProvider<WebMessage, ?> valueProvider,
            String header,
            String width,
            String key,
            boolean sortable,
            boolean visible) {
        Column<WebMessage> column =
                addColumn(valueProvider).setHeader(header).setKey(key).setWidth(width).setSortable(sortable);
    ...
    private CallbackDataProvider<WebMessage, WebMessage> getCallbackDataProvider() {
        if (callbackDataProvider == null) {
            callbackDataProvider = new CallbackDataProvider<WebMessage, WebMessage>(
                    this::fetchCallback,
                    this::countCallback
            );
        }
