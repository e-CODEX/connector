 ...
 * link:../ui/configuration/link_configuration.html#link-configuration[Link UI Configuration Manual]
...
The links are implemented by plugins. A link is built by a link configuration and a link partner configuration.
So a link configuration consists of multiple link partners.
...
If a link configuration is started this is usually done within in a separate spring application context (child context). This approach reduces the visible of controller beans to child context beans. It also helps to implement a start and stop logic  for link configurations and link partners.
...
| This column holds an id of a transport system like JMS, CXF, ...
