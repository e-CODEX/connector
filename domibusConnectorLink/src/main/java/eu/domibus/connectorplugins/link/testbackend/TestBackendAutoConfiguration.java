...
public class TestBackendAutoConfiguration {
    ...
    public void init() {
        ...
        if (b) {
            ...
            domibusConnectorLinkPartner.setLinkPartnerName(new DomibusConnectorLinkPartner.LinkPartnerName(
            ...
            LOGGER.info(
                    ...
                    "Connector2Connector test plugin is enabled. Because profile [{}] is active and in at leas one "
                            + "business domain config connector2connector tests are enabled. Connector Test Backend "
                            + "will be activated!",
        ...
        for (DomibusConnectorBusinessDomain.BusinessDomainId laneId : activeBusinessDomainIds) {
            ...
            if (c2cTestProperties.isEnabled()) {
                ...
                if (c2cTestProperties.getAction() != null) {
                    rule = concatAnd(
                            rule,
                            String.format("equals(Action, '%s')", c2cTestProperties.getAction().getAction())
                    );
                }
                    ...
                    if (StringUtils.hasText(c2cTestProperties.getService().getName())) {
                        rule = concatAnd(
                                ...
                                String.format(
                                        ...
                                        c2cTestProperties.getService().getName()
                                )
                        );
                    }
                    if (StringUtils.hasText(c2cTestProperties.getService().getServiceType())) {
                        rule = concatAnd(
                                ...
                                String.format(
                                        ...
                                        c2cTestProperties.getService().getServiceType()
                                )
                        );
                    }
                }
                if (!StringUtils.hasText(rule)) {
                    String error = String.format(
                            "Cannot create RoutingRule for ConnectorTests. Check Config of BusinessDomain %s",
                            laneId
                    );
                ...
                try {
                    ...
                    routingRule.setDescription(
                            "Automatically by Connector2Connector Test created routing rule. To route connector " +
                                    "tests" + " messages to testbackend");
                    routingRule.setMatchClause(routingRulePattern);
                    LOGGER.info(
                            LoggingMarker.Log4jMarker.CONFIG,
                            "Adding the routing rule {} to backend routing rules for testbackend on business domain " + "[{}]",
                            routingRule,
                            laneId
                    );
                    ...
                    String error = String.format(
                            "Cannot create RoutingRule for ConnectorTests. The routing rule %s is illegal. Check " +
                                    "Config of BusinessDomain %s",
                            rule,
                            laneId
                    );
                    throw new IllegalArgumentException(error);
