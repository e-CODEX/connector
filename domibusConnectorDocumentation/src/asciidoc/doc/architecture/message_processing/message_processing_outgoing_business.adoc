...
Within the domibusConnectorLink, the message is transformed from the transition model of the API to the domibusConnector internal domain model. During this transformation the payload is handled by the link:../../architecture/payload_handling.html#payload-handling[Large File Persistence Service].
...
After the message is successfully put on the submitToLinkQueue, the SUBMISSION_ACCEPTANCE evidence is sent back to the backend where the business message came from (can be deactivated by configuration).
