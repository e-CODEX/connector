...
The postfix is optionally used to mark a build with an alpha state, release candidate or with RELEASE as a release build.

== Note on gateway compatibility

The connector is intended to be used with the domibus AS4 gateway from CEF. For this purpose the domibus-connector-plugin
