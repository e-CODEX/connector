...
public class HelperMethods {
    ...
    public boolean checkKeyIsLoadable(
        ...
        if (keyConfig == null || keyConfig.getAlias() == null || storeConfig == null) { // DO NOT CHECK IF keyConfig
            // or keyAlias or storeConfig is null
            LOGGER.trace("checkKeyIsLoadable skipped because either keyConfig, keyConfig.alias or storeConfig is " +
                                 "null!");
            return true;
        }
        LOGGER.trace(
                "checkKeyIsLoadable with context [{}], storeConfig [{}], keyConfig [{}]",
                context,
                storeConfig,
                keyConfig
        );
        ...
        try {
            ...
            if (key != null) {
                ...
                String error =
                        String.format("Cannot retrieve key with alias [%s] and password [%s]! ", alias, logPassword);
                context.buildConstraintViolationWithTemplate(error).addConstraintViolation();
            }
        } catch (KeyStoreException e) {

            String error = String.format("key with alias [%s] and pw [%s] could not recovered! KeyStoreException!",
                                         alias,
                                         logPassword
            );
            LOGGER.warn(error, e);
