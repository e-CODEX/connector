...
public class TokenIssuerChecker extends AbstractChecker<TokenIssuer> {
    ...
    private static Set<String> ISO_COUNTRIES_LOOKUP;

    static {
        final String[] countries = Locale.getISOCountries();
        ISO_COUNTRIES_LOOKUP = new HashSet<String>(countries == null ? 1 : countries.length);
        if (countries != null) {
    ...
    public CheckResult run(final TokenIssuer object) {
        ...
        if (StringUtils.isEmpty(object.getCountry())) {
            ...
            if (!ISO_COUNTRIES_LOOKUP.contains(object.getCountry().toLowerCase())) {
                detect(r, true, "the token issuer's attribute 'country' is not a valid (is must be a 2-letter country code defined in ISO 3166)");
            }
        }
