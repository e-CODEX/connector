    ...
    private LegalValidationResult createImpl(final Token token) throws ECodexException {
        ...
        if(token.getAdvancedElectronicSystem() == AdvancedSystemType.AUTHENTICATION_BASED) {
        	if(signatures != null && !signatures.isEmpty() && signatures.size() == 1 &&
                ...
                result.setDisclaimer("e-CODEX does not approve the validity of the document. There are too many signatures of authentication service providers present.");        		
                ...
                result.setTrustLevel(LegalTrustLevel.SUCCESSFUL);
                result.setDisclaimer("e-CODEX approves the validity of the document. It is attested that it fulfils the requirements to be legally valid in the sending country.");       		
        	}else{
                result.setTrustLevel(LegalTrustLevel.NOT_SUCCESSFUL);
                result.setDisclaimer("e-CODEX does not approve the validity of the document. The signatory of the document could not be verified as being a valid authentication service provider!");
        	}      	
        } else {
        	...
        	if (invalidCount == 0 && validCount == 0) {
                ...
                result.setTrustLevel(LegalTrustLevel.NOT_SUCCESSFUL);
                result.setDisclaimer("e-CODEX cannot approve the validity of the document as there are valid and invalid signatures present on the document. Please check the technical validation report!");
