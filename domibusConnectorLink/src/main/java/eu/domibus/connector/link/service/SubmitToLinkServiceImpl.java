...
public class SubmitToLinkServiceImpl implements SubmitToLinkService {
    ...
    public void submitToLink(DomibusConnectorMessage message) throws DomibusConnectorSubmitToLinkException {
        ...
        if (direction.getTarget() == MessageTargetSource.BACKEND) {
            ...
            linkPartnerName = new DomibusConnectorLinkPartner.LinkPartnerName(message.getMessageDetails()
            ...
            linkPartnerName =
                    new DomibusConnectorLinkPartner.LinkPartnerName(message.getMessageDetails().getGatewayName());
        } else {
        ...
        if (submitToLinkPartner.isPresent()) {
            ...
            String errorMessage =
                    String.format("The LinkPartner with name [%s] could not be found/is not active!", linkPartnerName);
