...
public class Reports extends VerticalLayout {
    ...
    public Reports(@Autowired WebReportsService reportsService) {
        ...
        includeEvidences.addValueChangeListener(e -> includeEvidencesValue =
                includeEvidences.getValue().booleanValue());
    ...
    private void generateReport() {
        List<WebReportEntry> generatedReport =
        ...
        if (!CollectionUtils.isEmpty(generatedReport)) {
            ...
            for (WebReport report : sortReport) {
                ...
                Column<WebReportEntry> partyCol =
                        grid.addColumn(WebReportEntry::getParty).setHeader("Party").setWidth("200px");
                Column<WebReportEntry> serviceCol =
                        grid.addColumn(WebReportEntry::getService).setHeader("Service").setWidth("300px");
                Column<WebReportEntry> receivedCol =
                        grid.addColumn(WebReportEntry::getReceived).setHeader("Messages received from")
                            .setWidth("300px");
                Column<WebReportEntry> sentCol =
                        grid.addColumn(WebReportEntry::getSent).setHeader("Messages sent to").setWidth("300px");
            ...
            download.addClickListener(e -> {
                ...
                UI.getCurrent().getElement().appendChild(oName.getElement(), file, dummy);
