...
This contains an example deployment on a Centos 7 system.
...
----
sudo yum install -y java-21-openjdk-headless mariadb tomcat mysql-connector-java
----

* java-21-openjdk-headless java runtime environment
...
By linking you get the benefit, that the driver is updated by the Centos distribution.
____
...
If the database has started successfully log into the database with the following command
...
----
and configure the properties. You might enable liquibase for database initialisation by setting
----
    spring.liquibase.enabled=true
----
otherwise you have to apply the database scripts to the database.
...
NOTE: if you plan to also install domibus (AS4 Gateway) make sure that you are using a different tomcat!
