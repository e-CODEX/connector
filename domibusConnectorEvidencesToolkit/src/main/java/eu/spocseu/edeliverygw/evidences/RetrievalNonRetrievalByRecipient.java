...
public class RetrievalNonRetrievalByRecipient extends Evidence {
    private static Logger LOG = LoggerFactory.getLogger(RetrievalNonRetrievalByRecipient.class);
     ...
     */
    public RetrievalNonRetrievalByRecipient(EDeliveryDetails details, Evidence evidence) {
        super(details);
     ...
     */
    public RetrievalNonRetrievalByRecipient(EDeliveryDetails details, Evidence evidence, boolean isAcceptance) {
        super(details);
    ...
    //	}
    // klara
    public RetrievalNonRetrievalByRecipient(EDeliveryDetails details, Evidence evidence, EventReasonType eventReson) {
        super(details);
        init(details, evidence, false);
        super.setEventReason(eventReson);
    }
    ...
    public RetrievalNonRetrievalByRecipient(
            EDeliveryDetails details, REMEvidenceType evidenceType, boolean isAcceptance) {
    ...
    public RetrievalNonRetrievalByRecipient(
            ...
            InputStream evidenceSream,
            Evidences typeOfInputStream) throws SpocsWrongInputDataException {
        super(details);
        if (typeOfInputStream.equals(Evidences.DELIVERY_NON_DELIVERY_TO_RECIPIENT))
            init(details, new DeliveryNonDeliveryToRecipient(details, evidenceSream), true);
        if (typeOfInputStream.equals(Evidences.RELAY_REM_MD_ACCEPTANCE_REJECTION))
            init(details, new RelayREMMDAcceptanceRejection(details, evidenceSream), true);
    }

    private void init(EDeliveryDetails details, Evidence previousEvidence, boolean isAcceptance) {
        ...
        if (isAcceptance) {
            ...
            setEventCode(Evidences.RETRIEVAL_NON_RETRIEVAL_BY_RECIPIENT.getSuccessEventCode());
        } else {
            LOG.debug("Create RetrievalNonRetrievalByRecipient in fault case.");
            setEventCode(Evidences.RETRIEVAL_NON_RETRIEVAL_BY_RECIPIENT.getFaultEventCode());
        }
    ...
    private void init(
            EDeliveryDetails details, REMEvidenceType previousEvidence, boolean isAcceptance) {
        evidenceType = Evidences.RETRIEVAL_NON_RETRIEVAL_BY_RECIPIENT;
        if (isAcceptance) {
            LOG.debug("Create RetrievalNonRetrievalByRecipient in success case.");
            setEventCode(Evidences.RETRIEVAL_NON_RETRIEVAL_BY_RECIPIENT.getSuccessEventCode());
    ...
    public void serialize(OutputStream out) throws JAXBException {
        JaxbContextHolder.getSpocsJaxBContext().createMarshaller()
                         .marshal(new ObjectFactory().createRetrievalNonRetrievalByRecipient(jaxbObj), out);
    }
