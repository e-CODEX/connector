...
public class SpocsFragments {
    private static final Logger LOG = LoggerFactory.getLogger(SpocsFragments.class);
    ...
    public static EntityDetailsType createEntityDetails(
            NamePostalAddressType postalAddress, AttributedElectronicAddressType electronicAddressType) {
        ...
        entitiyDetails.getAttributedElectronicAddressOrElectronicAddress().add(electronicAddressType);
    ...
    public static void validateElectronicAdress(
            AttributedElectronicAddressType electronicAddress) throws MalformedURLException {
        try {
            new InternetAddress(electronicAddress.getValue(), true);
        } catch (AddressException e) {
            throw new MalformedURLException("Electronic address not valid! Address: " + electronicAddress.getValue());
        }
    }
     ...
     * @return
     */
    public static AttributedElectronicAddressType getFirstElectronicAddressWithURI(EntityDetailsType jaxbObj) {
        AttributedElectronicAddressType eAdress =
                (AttributedElectronicAddressType) jaxbObj.getAttributedElectronicAddressOrElectronicAddress().get(0);
        if (eAdress.getValue() != null) return eAdress;
        else {
    ...
    public static NamePostalAddressType getFirstNamePostalAddressType(
        ...
        Iterator<NamePostalAddressType> it = jaxbObj.getNamesPostalAddresses().getNamePostalAddress().iterator();
        while (it.hasNext()) {
     ...
     *
     * @param date The date/time that is to be included in the
     ...
     * @throws DatatypeConfigurationException If there are converting errors with the date objects.
     */
    public static XMLGregorianCalendar createXMLGregorianCalendar(int deltaDays) throws DatatypeConfigurationException {
        GregorianCalendar cal = new GregorianCalendar();
    ...
    public static AttributedElectronicAddressType getAttributedElectronicAdress(
        ...
        List<Object> elecAdress = details.getAttributedElectronicAddressOrElectronicAddress();
