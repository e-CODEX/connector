...
[[connector-link-plugins]]
= Connector Link Plugins Manual

Since 4.3.0 the connector comes with different link plugins.

This document describes how these links are configured with properties. The UI configuration manual is link:../ui/configuration/link_configuration.html#link-configuration[here] available.

The general architecture of the link is link:../architecture/link_partners.html#link-partners[here] available.
...
to the message processing. So how/who the LinkPartner is  completely
transparent to the message processing flow.

The configuration itself is read into the DomibusConnectorLink and
DomibusConnectorLinkPartner classes. Afterward when the LinkPartner and Link is
...
CAUTION: The backends are authenticated with certificates at the connector. The CN of the certificate is used to determine which backend is talking to the connector. So make sure every backend has its own CN within its certificate.
