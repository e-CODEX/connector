...
Before the message is submitted to the queue, it is transformed to the internal domain model. During this transformation, the payload is handled by the link:../../architecture/payload_handling.html#payload-handling[Large File Persistence Service].
