...
public class BusinessContentChecker extends AbstractChecker<BusinessContent> {
    ...
    public CheckResult run(final BusinessContent object) {
        ...
        final Set<String> docNames = new HashSet<String>();
    ...
    private void checkCompliance(final CheckResult r, final Set<String> docNames, final String docReference, final DSSDocument document) {
        if (document == null) {
        ...
        // check that it has not the name of any additional file, that will be put in the SignedContent.zip by the library
        ...
        if (checkDuplicates) {
            if (docNames.contains(nameRaw)) {
                detect(r, false, "the document [" + docReference + "] has the name '" + nameRaw + "' that already been used before and the previous document will be overwritten in the final asic-container.");
            }
            final String nameNormalised = nameRaw.toUpperCase();
            if (!nameRaw.equals(nameNormalised)) {
                if (docNames.contains(nameNormalised)) {
                    detect(r, false, "the document [" + docReference + "] has the name '" + nameRaw + "' that already been used before with another case (upper and/or lower) and it is likely that this may lead to some confusion.");
                }
            }
