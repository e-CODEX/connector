    ...
    public boolean isValid(
        ...
        if (allowedAdvancedSystemTypes.contains(AdvancedElectronicSystemType.SIGNATURE_BASED) &&
            ...
            context.buildConstraintViolationWithTemplate(
                           "AllowedAdvancedSystemTypes contains SIGNATURE_BASED so signature-validation must be " +
                                   "configured"
                   )
        ...
        if (allowedAdvancedSystemTypes.contains(AdvancedElectronicSystemType.AUTHENTICATION_BASED) &&
            ...
            context.buildConstraintViolationWithTemplate(
                           "AllowedAdvancedSystemTypes contains AUTHENTICATION_BASED so authentication-validation " +
                                   "must" +
                                   " be configured"
                   )
        ...
        if (defaultAesSystem == null) {
            context.buildConstraintViolationWithTemplate(
                           "The DefaultAdvancedSystemType must be set to one of the AllowedAdvancedSystemTypes, but " +
                                   "it" +
                                   " is not set at all!"
                   )
        ...
        if (!allowedAdvancedSystemTypes.contains(defaultAesSystem)) {
            context.buildConstraintViolationWithTemplate(
                           "The DefaultAdvancedSystemType must be set to one of the AllowedAdvancedSystemTypes!"
                   )
                   .addPropertyNode("defaultAdvancedSystemType")
                   .addConstraintViolation();
            valid = false;
