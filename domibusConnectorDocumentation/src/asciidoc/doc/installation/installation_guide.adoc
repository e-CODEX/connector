...
This chapter describes what has to be in place prior to installing the domibusConnector.It also lists some technical specifications of the domibusConnector to give a more detailed insight.
...
During the implementation and testing phase of the domibusConnector, it was installed and tested in the following environments:
...
link:../versioning_and_compatibility/testing_matrix.html#testing-matrix[Testing Matrix]
...
https://scm.ecodex.eu/artifactory/libs-release/eu/domibus/connector/domibusConnectorDistribution/{project-version}/
...
|This guide, as well as some other important documentation, are here as PDF documents.
...
We recommend creating an own schema/user for the domibusConnector database.

=== Supported database vendors

You can find the list of supported databases in the previous section xref:_database[Database].
...
A more detailed description of the certificates and stores and how they can be configured is on page link:certificates.html#certificates[certificates].
...
The properties in those files are all well described on what is expected there.
...
The variants on how to set those environment variables on your web server environment are dependent on what product you have in place.
For the web server products Apache Tomcat this is described as exemplary in the Chapter xref:_deployment[Deployment].
...
* Apache Tomcat 10.1.30 on Windows 10
==== Deployment steps
...
===== Linux (Ubuntu)
----
export CATALINA_HOME=/opt/tomcat-10.1.30

export JAVA_OPTS="$JAVA_OPTS -Dconnector.config.file=$CATALINA_HOME/conf/connector/connector.properties"
export JAVA_OPTS="$JAVA_OPTS -Dconnector.logging.config=$CATALINA_HOME/conf/connector/log4j2.xml"
----
