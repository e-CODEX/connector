...
public abstract class Evidence {
    ...
    protected void initEvidenceIssuerDetailsWithEdeliveryDetails(EDeliveryDetails _details) {
        ...
        try {
            jaxbObj.setEventTime(SpocsFragments.createXMLGregorianCalendar(new Date()));
    ...
    public EventReasonType getEventReson() {
        if (jaxbObj.getEventReasons() == null) return null;
        else {
            List<EventReasonType> reasons = jaxbObj.getEventReasons().getEventReason();

            if (reasons != null && reasons.size() == 1) return reasons.get(0);
            else return null;
        }
    }
    ...
    protected REMEvidenceType createRemEvidenceType(
            EDeliveryDetails details, String senderEAddress, String recipientAddress) {
        ...
        try {
            remEvi.setEventTime(SpocsFragments.createXMLGregorianCalendar(new Date()));
        } catch (DatatypeConfigurationException e) {
            LOG.error("Date error:" + e.getMessage());
        }
        remEvi.setSenderDetails(createEntityDetailsType(senderEAddress));
        EntityDetailsListType detailList = new EntityDetailsListType();
        detailList.getEntityDetails().add(createEntityDetailsType(recipientAddress));
    ...
    protected EntityDetailsType createEntityDetailsType(String eAddress) {
        ...
        return createEntityDetailsType(eAddress, null, null, null, null, null, (String) null);
    }

    protected EntityDetailsType createEntityDetailsType(String eAddress, String displayName, String postalName) {
        String[] array = {postalName};
        return createEntityDetailsType(eAddress, displayName, null, null, null, null, array);
    }

    protected EntityDetailsType createEntityDetailsType(String eAddress, String displayName, String[] postalName) {
        return createEntityDetailsType(eAddress, displayName, null, null, null, null, postalName);
    }
    ...
    protected EntityDetailsType createEntityDetailsType(
            ...
            String postalName) {
        String[] array = {postalName};
        return createEntityDetailsType(eAddress, displayName, street, locality, zipcode, country, array);
    }
    ...
    protected EntityDetailsType createEntityDetailsType(
        ...
        if (postalName != null) {
            ...
            if (street != null) postalAddressType.getStreetAddress().add(street);
        ...
        if (eAddress != null) {
            ...
            // .name());
            detailsType.getAttributedElectronicAddressOrElectronicAddress().add(elAddre);
        }
        return detailsType;
    }
