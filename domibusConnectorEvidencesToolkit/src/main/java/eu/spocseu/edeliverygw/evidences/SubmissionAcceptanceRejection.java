...
public class SubmissionAcceptanceRejection extends Evidence {
    private static Logger LOG = LoggerFactory.getLogger(SubmissionAcceptanceRejection.class);
    ...
    protected SubmissionAcceptanceRejection(
            EDeliveryDetails details, boolean isAcceptance) {
        super(details);
        if (isAcceptance) {
     ...
     * @param dispatch     DispatchMessage as input information for this object. This is
    ...
    public SubmissionAcceptanceRejection(
            EDeliveryDetails details, REMDispatchType dispatch, boolean isAcceptance) {
        super(details);
    ...
    private void initWithDispatch(
        ...
        EntityDetailsType from = dispatch.getMsgMetaData().getOriginators().getFrom();
        String senderEAddress =
                ((AttributedElectronicAddressType) from.getAttributedElectronicAddressOrElectronicAddress()
                                                       .get(0)).getValue();

        String senderName = ((AttributedElectronicAddressType) from.getAttributedElectronicAddressOrElectronicAddress()
                                                                   .get(0)).getDisplayName();

        String[] senderPostalName = null;
        if (dispatch.getMsgMetaData().getOriginators().getFrom().getNamesPostalAddresses() != null) {
            senderPostalName = new String[dispatch.getMsgMetaData().getOriginators().getFrom().getNamesPostalAddresses()
                                                  .getNamePostalAddress().get(0).getEntityName().getName().size()];

            dispatch.getMsgMetaData().getOriginators().getFrom().getNamesPostalAddresses().getNamePostalAddress().get(0)
                    .getEntityName().getName().toArray(senderPostalName);
        }

        jaxbObj.setSenderDetails(createEntityDetailsType(senderEAddress, senderName, senderPostalName));
        ...
        String recipientAddress = SpocsFragments.getAttributedElectronicAdress(recipient).getValue();
        String recipientName = SpocsFragments.getAttributedElectronicAdress(recipient).getDisplayName();
        String[] recipientPostalNames = new String[0];

        if (dispatch.getMsgMetaData().getDestinations().getRecipient().getNamesPostalAddresses() != null && dispatch
                .getMsgMetaData().getDestinations().getRecipient().getNamesPostalAddresses().getNamePostalAddress()
                .get(0) != null) {
            recipientPostalNames = dispatch.getMsgMetaData().getDestinations().getRecipient().getNamesPostalAddresses()
        ...
        detailList.getEntityDetails()
                  .add(createEntityDetailsType(recipientAddress, recipientName, recipientPostalNames));
        jaxbObj.setRecipientsDetails(detailList);
        jaxbObj.setSubmissionTime(dispatch.getMsgMetaData().getDeliveryConstraints().getInitialSend());
        if (dispatch.getMsgMetaData().getOriginators() != null && dispatch.getMsgMetaData().getOriginators()
                                                                          .getReplyTo() != null && dispatch
                .getMsgMetaData().getOriginators().getReplyTo()
                .getAttributedElectronicAddressOrElectronicAddress() != null) {
            AttributedElectronicAddressType eAddress =
                    SpocsFragments.getFirstElectronicAddressWithURI(dispatch.getMsgMetaData().getOriginators()
                                                                            .getReplyTo());
            jaxbObj.setReplyToAddress(eAddress);
        } else if (dispatch.getMsgMetaData().getOriginators() != null) {
            AttributedElectronicAddressType eAddress =
                    SpocsFragments.getAttributedElectronicAdress(dispatch.getMsgMetaData().getOriginators().getFrom());
            jaxbObj.setReplyToAddress(eAddress);
        }
    ...
    public void serialize(OutputStream out) throws JAXBException {
        ...
        Marshaller m = JaxbContextHolder.getSpocsJaxBContext().createMarshaller();
