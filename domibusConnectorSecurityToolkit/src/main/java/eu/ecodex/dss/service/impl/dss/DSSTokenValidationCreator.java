 ...
 * /DSSTokenValidationCreator.java $
 * $Revision: 1904 $
...
class DSSTokenValidationCreator {
     ...
     */
    private static final ThreadLocal<SoftReference<DecisionData>> DATA_CACHE =
            new ThreadLocal<SoftReference<DecisionData>>();
     ...
     * @param detachedSignature   the optional detached signature document; if present this will be used to provide
     *                            the signature
     ...
     * see also <a href="http://www.jira.e-codex.eu/browse/ECDX-25">http://www.jira.e-codex.eu/browse/ECDX-25</a>
    ...
    void run() throws Exception {
        ...
        try {
            ...
            if (tValidation.getVerificationTime() == null) {
                tValidation.setVerificationTime(
    ...
    private void runImpl() throws Exception {
        ...
        //		final InputStream resourceAsStream = DSSECodexContainerService.class.getResourceAsStream
        //		("/validation/102853/constraint.xml");
        ...
        if (simpleReport.getValidationTime() != null) {
            ...
            LOG.lInfo(
                    "no time-information/verification-time found in DSS validation report. setting the current " +
                            "datetime.");
        ...
        for (AdvancedSignature curSignature : signatures) {
            if (simpleReport.getSignatureFormat(curSignature.getId()) != null && simpleReport
                    .getSignatureFormat(curSignature.getId()).getSignatureForm().equals(SignatureForm.XAdES)) {
        ...
        // LOG.lDetail("using the signature information at index {} in the DSS validation report for further
        // determination: {}", signatures.indexOf(lastSignature), lastSignature);
        ...
        for (AdvancedSignature curSignature : signatures) {
            if (ignoredCertificateStore != null && !ignoredCertificateStore
                    .getByPublicKey(curSignature.getSigningCertificateToken().getCertificate().getPublicKey())
                    .isEmpty()) {
                LOG.lDetail(
                        "Removing curSignature [{}] because the according certificate is within ignore list",
                        curSignature
                );
                ...
                // structure verification is always true if the DSS report is generated and contains signature
                // information.
    ...
    private DecisionData computeData(final AdvancedSignature signature, Reports reports) throws Exception {
        ...
        if (signingCertificateToken.isSelfSigned()) {
            issuerCertificateWrapper = TechnicalValidationUtil.getCertificateWrapper(
        ...
        final CertificateToken issuingCertificateToken =
                TechnicalValidationUtil.getCertificateToken(signature, issuerCertificateWrapper.getId());
        ...
        final boolean validSignatureConclusion = TechnicalValidationUtil.checkSignatureConclusion(
                simpleReport,
        ...
        final TechnicalTrustLevel validSignatureCertStatus = TechnicalValidationUtil.checkCertificateRevocation(
        ...
        final DiagnosisData diagnosis = new DiagnosisData(
                signingTime,
                signingCertificate,
                signingCertificateSubject,
                signingCertificateIssuer,
    ...
    private void determineDecision(DecisionData decisionData) {
        ...
        // 1. The signature has to be mathematical correct. Otherwise the result of the technical validation has to be
        // RED
        ...
        // 2. If DSS can recognize and analyze the signature format (e.g. PAdES-BES), the final conclusion can be
        // GREEN, otherwise the final conclusion will be RED
        ...
        // 3. QES, ADES_QC and ADES are allowed signatures levels and can create a GREEN result, an UNDETERMINED
        // signature level will not be allowed and the final conclusion will be RED
        ...
        // 4. The signing certificate at least has to be valid at the time of signing (not revoked, not expired,
        // recognizable by DSS).
        if (decisionData.validation.signatureCertStatus == TechnicalTrustLevel.FAIL || decisionData.validation.signatureCertHistory == TechnicalTrustLevel.FAIL) {
            decide(
                    decisionData,
                    TechnicalTrustLevel.FAIL,
                    "The signature certificate is not valid at the time of signing (non-active or revoked)."
            );
            return;
        }
        ...
        //    with the addition, that the issuing certificate had to be valid at the time the signing certificate
        //    started to become valid.
        ...
        if (decisionData.validation.issuerCertHistory != TechnicalTrustLevel.SUCCESSFUL) { // there is no SUFFICIENT
            // for this check
            decide(
                    decisionData,
                    TechnicalTrustLevel.FAIL,
                    "The issuer certificate is not valid at the time of signing (revoked, expired or not " +
                            "recognisable)."
            );
            return;
        }
        ...
        // 4-3. Being valid at the time of signing with an CRL and/or an OCSP being defined, but none of them is
        // reachable: YELLOW
        if (decisionData.validation.signatureCertStatus == TechnicalTrustLevel.SUFFICIENT || decisionData.validation.signatureCertHistory == TechnicalTrustLevel.SUFFICIENT) {
            decide(
                    decisionData,
                    TechnicalTrustLevel.SUFFICIENT,
                    "The signature certificate's validity at the time of signing could not be fully determined " +
                            "(OCSP/CRL data not available)."
            );
            return;
        }

        if (decisionData.diagnosis.signatureConclusion == SignatureQualification.QESIG) {
            // 5-1. In case of qualified signatures, the issuing certificate has to be present and verifiable at a
            // national TSL.
            //      Otherwise the signature can be assessed with “AdES_QC” and the comment “Unable to verify the
            //      certificates issuer at a national TSL”.
            if (!decisionData.validation.trustAnchor) {
                final SignatureAttributes tokenSignatureAttributes =
                        tValidation.getVerificationData().getSignatureData().get(0).getSignatureInformation();
            ...
            if (decisionData.diagnosis.signatureConclusion == SignatureQualification.ADESIG_QC || decisionData.diagnosis.signatureConclusion == SignatureQualification.ADESIG_QC) {
                // only for clarity
                // 5-2. In case of AdES and AdES_QC, the issuing certificate should be validated against a TSL if
                // possible
                //      and the result of the verification should be part of the technical validation part of the
                //      “Trust Ok”-Token.
    ...
    private void decide(final DecisionData data, final TechnicalTrustLevel level, final String comments) {
        ...
        if (curLevel == null ||
                (curLevel.equals(TechnicalTrustLevel.SUCCESSFUL) && (level.equals(TechnicalTrustLevel.SUFFICIENT) || level.equals(
                        TechnicalTrustLevel.FAIL))) ||
    ...
    public static class DiagnosisData {
        ...
        DiagnosisData(
                ...
                final TechnicalTrustLevel trustLevel,
    ...
    public static class ValidationData {
         ...
         */
        ValidationData(
                final boolean signatureComputation,
                final boolean signatureConclusion,
                final boolean signatureFormat,
                final TechnicalTrustLevel signatureCertStatus,
                final TechnicalTrustLevel signatureCertHistory,
                final boolean trustAnchor,
    ...
    public static class SignatureTimeComparator implements Comparator<AdvancedSignature> {
        ...
        /**
         * creates a copy of the list in parameter and sort that using the
         * {@link eu.ecodex.dss.service.impl.dss.DSSTokenValidationCreator.SignatureTimeComparator} singleton
         *
