...
| Is provided by the connector and is meant for the clients/connector backends to retrieve and push messages.
| 4.0
...
| messageConfirmations | DomibusConnectorMessageConfirmationType | Holds a list of confirmations (evidences). A business message can contain multiple evidences. If the messageContent is empty there must be exactly one messageConfirmation. In this case the message is an evidence message.
...
| resultMessage | xs:string | Freely chosen additional information. Should be used when result is false to return a meaningful error message.
...
| responseForMessageId | xs:string | This response should hold the by the sending component initially used message id in the following order: If provided the EBMS id, otherwise the backendMessageId.
...
| messageErrors | <<DomibusConnectorMessageErrorType>> | a list of occurred message errors
...
It should be a PDF-A document, to always make it possible for an e-Codex receiver to have at least one document which
...
field is used. It depends on the usecase if this id might match the eg. casenumber of a case. This would be true
...
If the message comes from a national message system, this would be the senders (technical) national address.
...
The connector has the AS4 configuration in place. So if a party or service is provided with insufficient information.
...
to put the PDF document and the detached Signature separately into the connector.
...
use case with an evidence message. This can be done by sending an evidence trigger to the connector. The
...
with a message id to a new delivered message
...
After the evidence was triggered the generated evidence message will not only be submitted to the other party
