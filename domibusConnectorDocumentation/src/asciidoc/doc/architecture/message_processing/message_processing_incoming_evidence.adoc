...
This chapter shows the example of an incoming evidence message. That means, that an evidence message is delivered
by the gateway.
...
[plantuml,message_proc_in_evidence_1,format=svg]
----
@startuml
...
[plantuml,message_proc_in_evidence_3,format=svg]
----
@startuml
