...
public class TransportStateMonitoringView extends DCVerticalLayoutWithTitleAndHelpButton implements AfterNavigationObserver {
    ...
    private final MultiSelectListBox<DomibusConnectorLinkPartner.LinkPartnerName> linkPartnerSelectBox =
    ...
    private Set<TransportState> filterForState =
            Stream.of(TransportState.values())
    ...
    private void initUI() {
        ...
        paginatedGrid
                .addColumn("attempt");
        paginatedGrid
                .addColumn(s -> s.getLastStatusUpdate() == null ? "" : s.getLastStatusUpdate().getTransportState()
                                                                        .name())
    ...
    private HorizontalLayout buttonProvider(DomibusConnectorTransportStep step) {
        ...
        gotoBusinessMessageButton.addClickListener((e) -> MessageDetails.navigateTo(connectorMessageId));
        layout.add(gotoBusinessMessageButton);
        gotoBusinessMessageButton.setEnabled(
                connectorMessageId != null && msg.map(DomainModelHelper::isBusinessMessage).orElse(false)
        );
    ...
    private int countCallback(Query<DomibusConnectorTransportStep, DomibusConnectorTransportStep> tfQuery) {
        // TODO: introduce own count call on DB so not ALL items are read from DB or check if Pageable.ofSize(0) avoids
        // fetching items
        try {
            Page<DomibusConnectorTransportStep> stepByLastState =
                    getDomibusConnectorTransportSteps(Pageable.ofSize(1));
    ...
    private Stream<DomibusConnectorTransportStep> fetchCallback(
            Query<DomibusConnectorTransportStep,
                    DomibusConnectorTransportStep> tfQuery) {
        ...
        try {
            ...
            PageRequest pageRequest =
                    PageRequest.of(offset / paginatedGrid.getPageSize(), paginatedGrid.getPageSize(), sort);
            Page<DomibusConnectorTransportStep> domibusConnectorTransportSteps =
                    getDomibusConnectorTransportSteps(pageRequest);
    ...
    public void afterNavigation(AfterNavigationEvent event) {
        ...
        HashSet<DomibusConnectorLinkPartner.LinkPartnerName> allLinkPartners =
                new HashSet<>(transportStepPersistenceService.findAllLinkPartners());
