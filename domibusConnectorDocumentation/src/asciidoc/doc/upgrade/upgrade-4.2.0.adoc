...
There have also been multiple changes to the database regarding the support for loopback message sending
and first work on full support on multi use-case handling. For the database changes migration scripts are
...
the connector will apply the database schema changes automatically on the first startup! So in this case
...
messages delivered to a pull backend will be stored within the database instead of a message queue,
