...
import eu.ecodex.connector.persistence.spring.DomibusConnectorPersistenceProperties;
...
/**
 * This service uses jdbc to create an input / output stream with writing it to an byte array
...
public class LargeFilePersistenceServiceJpaImpl implements LargeFilePersistenceProvider {
    private static final Logger LOGGER =
        LoggerFactory.getLogger(LargeFilePersistenceServiceJpaImpl.class);
    ...
    public LargeFileReference getReadableDataSource(LargeFileReference bigDataReference) {
        ...
        try {
            ...
            JpaBasedLargeFileReference jpaBasedDomibusConnectorBigDataReference =
                new JpaBasedLargeFileReference(this);
            ...
            var error = String.format(
                "Cannot load big data with storage reference [%s]\nThe actual implementation "
                    + "expects a Long as storage ref key!",
                storageReference
            );
    ...
    public LargeFileReference createDomibusConnectorBigDataReference(
        ...
        LOGGER.trace(
            ...
            connectorMessageId, documentName
        );
    ...
    @Transactional(readOnly = false)
    public LargeFileReference createDomibusConnectorBigDataReference(
        InputStream in, String connectorMessageId, String documentName,
    ...
    public void deleteDomibusConnectorBigDataReference(LargeFileReference bigDataReference) {
        LOGGER.trace(
            "deleteDomibusConnectorBigDataReference: called to delete all data {}",
            bigDataReference
        );
    ...
    public Map<DomibusConnectorMessageId, List<LargeFileReference>> getAllAvailableReferences() {
        ...
        all.forEach(bigData -> {
            ...
            reference.setWriteable(false);
            reference.setStorageIdReference(Long.toString(bigData.getId()));
            reference.setCreationDate(
                ZonedDateTime.ofInstant(bigData.getCreated().toInstant(), ZoneId.systemDefault()));
    ...
    protected void saveOnClose(DbBackedOutputStream dbBackedOutputStream) {
        PDomibusConnectorBigData bigData =
            bigDataDao.findById(dbBackedOutputStream.storageReference.getId()).get();
    ...
    private long convertStorageIdReferenceToDbId(String storageRef) {
        try {
            ...
            var error = String.format(
                "The provided storage reference [%s] cannot be converted to a big data database "
                    + "reference",
                storageRef
            );
    ...
    private class DbBackedOutputStream extends ByteArrayOutputStream {
        ...
        public void flush() throws IOException {
            LOGGER.debug(
                "called flush on DbBackedOutputStream [{}] - flush is not implemented "
                    + "doing nothing!",
                this
            );
            super.flush();
        ...
        public String toString() {
            ...
            return String.format(
                "DbBackedOutputStream with bytes [%d] and with storageRef: [%s]", byteCount,
                storageReference
            );
        }
    }
    ...
    private static class JpaBasedLargeFileReference extends LargeFileReference {
        ...
        public JpaBasedLargeFileReference(
            LargeFilePersistenceServiceJpaImpl persistenceServiceJpa) {
