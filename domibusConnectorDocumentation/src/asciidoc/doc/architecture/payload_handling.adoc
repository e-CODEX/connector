...
The configuration is described here: link:../configuration/large_file_storage_configuration.html#large-file-storage-configuration[Large File Handling Configuration]

Streaming support means that the content of the file is streamed into the application and also written out of the application as a stream. This approach consumes much less heap space than storing the file content into a byte array as it is done by the JPABased implementation.

== Class diagram

The main entry point for using the storage system is the DCBusinessDomainPersistenceService. This services offers to create, get, delete a Storage reference.
...
2. Delete a file on disk
3. Mark the file as deleted in DB and commit DB change

=== JPABased Database Storage (Does NOT support streaming)

The JPA storage stores the content in the database. For this JPA mapping is used. The implementation needs to load the complete file into a byte array before writing it into the database. Also, when the content is read, the whole file is stored into a byte array. So this storage engine should not be used if large files are expected.
...
It might also be useful to support a bucket storage like S3. This could be a nice feature for HA environments.
