...
public class LargeFilePersistenceServiceImpl implements LargeFilePersistenceService {
    ...
    public void init() {
        ...
        LargeFilePersistenceProvider p = availableLargeFilePersistenceProvider
                .stream() // lookup for provider name first, and then for class name
                .filter(largeFilePersistenceProvider -> (!StringUtils.isEmpty(
                        defaultProviderName) && defaultProviderName.equals(
                        largeFilePersistenceProvider.getProviderName()) || (StringUtils.isEmpty(
                        defaultProviderName) && defaultLargeFileProviderClass != null && defaultLargeFileProviderClass.isAssignableFrom(
                        largeFilePersistenceProvider.getClass()))))
                .findFirst().orElse(null);
        if (p == null) {
            throw new RuntimeException(String.format(
                    "No LargeFilePersistenceProvider provider with Class [%s] or Name [%s] is registered as spring " +
                            "bean!\n" + "The following LargeFilePersistenceProvider are available:\n[%s]\nPlease " +
                            "consider updating the configuration property [%s]",
    ...
    public LargeFileReference getReadableDataSource(LargeFileReference bigDataReference) {
        return getProviderByLargeFileReference(bigDataReference).getReadableDataSource(bigDataReference);
    }
    ...
    @Override
    public LargeFileReference createDomibusConnectorBigDataReference(
            DomibusConnectorMessageId connectorMessageId, String documentName, String documentContentType) {
        return defaultLargeFilePersistenceProvider.createDomibusConnectorBigDataReference(
                connectorMessageId,
                documentName,
                documentContentType
    ...
    @Override
    public void deleteDomibusConnectorBigDataReference(LargeFileReference bigDataReference) throws
            LargeFileDeletionException {
        try {
            getProviderByLargeFileReference(bigDataReference).deleteDomibusConnectorBigDataReference(bigDataReference);
        } catch (Exception e) {
    ...
    public Map<DomibusConnectorMessageId, List<LargeFileReference>> getAllAvailableReferences() {
        return availableLargeFilePersistenceProvider
                .stream().map(provider -> {
                    ...
                    return new HashMap<DomibusConnectorMessageId,
                            List<LargeFileReference>>();
    ...
    private LargeFilePersistenceProvider getProviderByLargeFileReference(LargeFileReference bigDataReference) {
        ...
        return this.getProviderByName(storageProviderName).orElseThrow(() -> new RuntimeException(String.format(
                "No  LargeFilePersistenceProvider with name %s is available.\n" + "The following " +
                        "LargeFilePersistenceProvider are available:\n[%s]",
                storageProviderName,
                getAvailableStorageProviderAsStringWithNewLine()
        )));
    }
    ...
    private String getAvailableStorageProviderAsStringWithNewLine() {
        return availableLargeFilePersistenceProvider
                .stream().map(l -> l.getProviderName() + ":" + l.getClass())
