...
public class WsGatewayPluginConfigurationProperties {
    ...
    @ConfigurationDescription(
            "Specifies the address where the Gateway WebService should be published\n" + "the path specefied here is "
                    + "added to the path of the CXF-Servlet\n" + "(which is per default configured as /services - " +
                    "this " + "leads to the default URL of\n" + "'/services/gateway'"
    )
    private String gwDeliveryServicePublishAddress = "/gateway";
    @NotBlank
    @ConfigurationDescription(
            "The URL of the domibus connector pull gateway plugin webservice eg. <domibus url>/services/pull-gw"
    )
