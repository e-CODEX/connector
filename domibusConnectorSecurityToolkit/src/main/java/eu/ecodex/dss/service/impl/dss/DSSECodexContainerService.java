 ...
 * /DSSECodexContainerService.java $
 * $Revision: 2267 $
...
public class DSSECodexContainerService implements ECodexContainerService {
    ...
    private ECodexContainer addSignatureImpl(final ECodexContainer container) throws ECodexException {
        ...
        throwIfNull(
                signedContent,
                "The file " + ContainerFileDefinitions.SIGNED_CONTENT_REF + " can not be found in the asic container"
        );
        throwIfNull(
                signatures,
                "The file " + ContainerFileDefinitions.SIGNATURES_REF + " can not be found in the asic container"
        );
        ...
        try {
            ...
            final DSSDocument signedContentXAdES =
                    SigningUtil.signXAdES(signingParameters, signedContent, SignaturePackaging.DETACHED);
    ...
    private CheckResult checkImpl(final ECodexContainer container) throws Exception {
        ...
        final CheckResult checkIssuerResult =
                CHECKER_TOKEN_ISSUER.run(container.getToken() == null ? null : container.getToken().getIssuer());
        ...
        tokenDocument
                .getDigestMethod()
                .setAlgorithm(tokenDocument.getDigestMethod().getAlgorithm().replaceAll("-", ""));
        ...
        if (!Arrays.equals(documentDigest, tokenDigest)) {
            checkResult.addProblem(
                    true,
                    "The digested document from the token is not the same as the digested business document"
            );
        }
     ...
     */
    @Override
    public ECodexContainer receive(
            final InputStream asicInputStream,
            final InputStream tokenStream) throws ECodexException {
     ...
     * {@inheritDoc}
     * <p>
     * Attaches an additional XAdES signature (based on this class' signingparameters) for the signed content to the
     * signatures.xml.
     */
    @Override
    ...
    private ECodexContainer createImpl(
            final BusinessContent businessContent,
            final TokenIssuer issuer) throws Exception {
        ...
        final DSSDocument tokenPDF = PDFUtil.concatenate(
                ContainerFileDefinitions.TOKEN_PDF_REF,
                summaryLegal,
                summaryTechnical,
                appendixPDF
        );
        ...
        final OriginalValidationReportContainer originalValidationReportContainer =
    ...
    /**
     * create the token with the provided data: issuer, tokenvalidation, tokendocument (including the digest
     * algorithm/method/value)
     * this is also the invocation point for the technical and legal validations provided by the corresponding
     * configured services.
     *
    ...
    private Token createToken(
        ...
        throwIfNull(
                ...
                "the technical validation service did not create a technical validation result in the token " +
                        "validation" +
                        " object"
        );
        throwIfNull(
                tokenValidation.getTechnicalResult().getTrustLevel(),
                "the technical validation service did not create a trust-level in the technical validation result in " +
                        "the token validation object"
        );
        // store in the token
        ...
        if (issuer.getAdvancedElectronicSystem() == AdvancedSystemType.AUTHENTICATION_BASED &&
            ...
            if (sigList != null && !sigList.isEmpty()) {
                if (sigList.size() != 1) {
                    LOG.lWarn(
                            "Invalid number of authentication certificates detected: {} - Only one " +
                                    "signature/certificate is supported!",
                            sigList.size()
                    );
                    tokenValidation.getTechnicalResult().setTrustLevel(TechnicalTrustLevel.FAIL);
                    tokenValidation.getTechnicalResult()
                                   .setComment("Invalid count of signatures on the authentication data.");
                } else {
                    Signature curSig = sigList.get(0);
                    AuthenticationCertificate authenticationCertValidation =
                            ((DSSECodexTechnicalValidationService) technicalValidationService)
                                    .verifyAuthenticationCertificate(
                                            businessDocument,
                                            detachedSignature
                                    );
        ...
        // 2014/07/12 klara: Hotfix - Added "-" between SHA and the respective number for downward compatibility (e.g.
        // SHA256 becomes SHA-256)
        ...
        throwIfNull(
                ...
                "the legal validation service did not create a trustlevel in the legal validation result"
        );
        // store in the token
    ...
    private DSSDocument createAsicDocument(
            final BusinessContent businessContent,
            final DSSDocument trustOkTokenPDF,
        ...
        try {
            ...
            final String docName =
                    StringUtils.isEmpty(businessDocument.getName()) ? "businessdocument" : businessDocument.getName();
            ...
            for (int i = 0; i < attachments.size(); i++) {
                ...
                final String attachmentName =
                        StringUtils.isEmpty(attachment.getName()) ? ("attachment" + i) : attachment.getName();
            ...
            if (false) { // purposely excluded, because the xml shall be shipped in parallel to the asic-container
                // according to WP4 decision
    ...
    private ECodexContainer receiveImpl(
            final InputStream asicInputStream,
            final InputStream tokenStream) throws Exception {
        ...
        final DSSDocument tokenXML =
                new InMemoryDocument(DSSUtils.toByteArray(tokenStream), ContainerFileDefinitions.TOKEN_XML_REF);
        ...
        for (final DSSDocument asicEntry : asicDocumentList) {
            ...
            for (final DSSDocument signedContentEntry : zipDocumentList) {
                ...
                } else if (!StringUtils.isEmpty(businessDocumentName) && businessDocumentName.equalsIgnoreCase(
                        signedContentEntryName)) {
                    // Set the main document
                    businessContent.setDocument(signedContentEntry);
                } else if (!StringUtils.isEmpty(detachedSignatureName) && detachedSignatureName.equalsIgnoreCase(
                        signedContentEntryName)) {
