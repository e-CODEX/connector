...
The connector uses the link:{basepath}/doc/architecture/dss_tools.html[DSS library]. Which can be configured:
...
The basic dss configuration includes the location of tl cache. This cache is used to store downloaded trusted lists information.
...
A trust source can contain multiple Lists of Trusted List (LOTL) or trusted lists (TL). The name of the trust source can be used within the link:{basepath}/doc/ui/configuration/business_document_verification.html[business document validation configuration].
