...
class TechnicalValidationUtil {
     ...
     * @throws javax.xml.datatype.DatatypeConfigurationException as of the underlying
     *                                                           {@link DatatypeFactory}
     */
    public static XMLGregorianCalendar getSigningTime(
        final DiagnosticData diagnosticData, final String signatureId)
    ...
    public static XMLGregorianCalendar getSigningTime(
        final AdvancedSignature signature, final String signatureId)
        throws DatatypeConfigurationException {
    ...
    public static CertificateToken getSigningCertificateToken(final AdvancedSignature signature) {
        ...
        return signature.getSigningCertificateToken();
    }

    public static CertificateToken getCertificateToken(
        final AdvancedSignature signature, String certificateId) {
    ...
    public static CertificateWrapper getCertificateWrapper(
        ...
        if (usedCertificates == null || issuerName == null || issuerName.isEmpty()) {
            return null;
        }
     ...
     */
    public static CertificateToken getIssuerCertificateToken(
        ...
        return new CertificateToken(getIssuerCertificate(certificateToken));
    }
    ...
    /**
     * Get the signature level. If the signature information are incomplete , it return by default
     * UNDETERMINED.
     *
     * @param simpleReport The {@link SimpleReport}
     * @param signatureId  signature id
     ...
     * @return The result
     */
    public static boolean checkSignatureCorrectness(
        final SimpleReport simpleReport, final String signatureId) {
        if (simpleReport == null) {
            return false;
        }
        return simpleReport.isValid(signatureId);
    }

    /**
     * Search the signing certificate in the Certificate path revocation analysis, compare the two's
     * certificates and check if the status is valid.
     *
     * @param certificateToken The {@link CertificateToken}
     * @return The result (a {@link TechnicalTrustLevel}, where
     *     {@link TechnicalTrustLevel#SUFFICIENT} means that no online source could be contacted)
     */
    public static TechnicalTrustLevel checkCertificateRevocation(
        final CertificateToken certificateToken, boolean... revoked) {
        ...
        if (revoked.length == 0) {
            if (certificateToken.isSelfSigned()) {
                ...
                // 4-3. Being valid at the time of signing with an CRL and/or an OCSP being
                // defined, but none of them is reachable: YELLOW
    ...
    /**
     * Search the signing signatureId in the Certificate path revocation analysis, compare the two's
     * certificates and check the validity at singing time. Or: use directly the signatureId to
     * compute the result.
     *
     * @param certificateToken The {@link CertificateToken}
    ...
    public static TechnicalTrustLevel checkCertificateValidity(
        final CertificateToken certificateToken, XMLGregorianCalendar signingTime) {

        if (certificateToken == null) {
            return TechnicalTrustLevel.FAIL;
        }
        final boolean valid =
            certificateToken.isValidOn(signingTime.toGregorianCalendar().getTime());
    ...
    public static boolean checkTrustAnchor(DiagnosticData diagnosticData, String certificateId) {
        ...
        for (eu.europa.esig.dss.diagnostic.jaxb.XmlCertificate xmlCertificate : certificates) {
            if (xmlCertificate.getId() != null && xmlCertificate.getId().equals(certificateId)
                && xmlCertificate.isTrusted()) {
                return true;
            }
        }
            ...
            case NA:
                if (detailedReport != null) {
                    ...
                    XmlValidationSignatureQualification signatureQualification =
                        (signature == null) ? null :
                            signature.getValidationSignatureQualification();
                    List<XmlConstraint> signatureConstraints =
                        (signatureQualification == null) ? null :
                            signatureQualification.getConstraint();
                    ...
                    if (signatureConstraints == null || signatureConstraints.isEmpty()) {
                        ...
                        for (XmlConstraint curConstraint : signatureConstraints) {
                            XmlMessage curName =
                                (curConstraint == null) ? null : curConstraint.getName();
                            String nameId = (curName == null) ? null : curName.getValue();

                            XmlStatus curStatus =
                                (curConstraint == null) ? null : curConstraint.getStatus();
                            String finalStatus = (curStatus == null) ? null : curStatus.value();

                            if (nameId != null && nameId.equals("QUAL_IS_ADES")
                                && finalStatus != null && finalStatus.equals("OK")) {
                                return true;
                            } else if (nameId != null && nameId.equals("QUAL_IS_ADES_IND")
                                && finalStatus != null  && finalStatus.equals(
                                "OK")) {
                                return true;
                            }
                        }
