...
| AS4-Gateway | Domibus | eDelivery | AS4 message creation, signing AS4-Message, fulfilling eDelivery Policy
...
* Every UseCase is within the same P-Mode set. This means that the merged usecases must not use the same name for a p-Mode item.
...
** The OriginalSender, FinalRecipient has to follow a pattern e.g.: {{appid}::{{nationalpart}
