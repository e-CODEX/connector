...
public class UserList extends VerticalLayout implements AfterNavigationObserver {
    ...
    public UserList(@Autowired WebUserService service, @Autowired UserDetails userDetails) {
        ...
        grid.addColumn(WebUser::getUsername).setHeader("Username").setWidth("250px").setSortable(true)
            .setResizable(true);
        grid.addColumn(WebUser::getRole).setHeader("Role").setWidth("70px").setSortable(true).setResizable(true);
        grid.addComponentColumn(webUser -> getCheckboxForList(webUser.isLocked())).setHeader("Locked").setWidth(
                "30px");
    ...
    private void filter() {
        ...
        for (WebUser msg : fullList) {
            if ((
                    usernameFilterText.getValue().isEmpty() || (msg.getUsername() != null
                            && msg.getUsername().toUpperCase().contains(usernameFilterText.getValue().toUpperCase()))
