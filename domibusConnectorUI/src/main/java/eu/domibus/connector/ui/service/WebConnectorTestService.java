...
public class WebConnectorTestService {
    ...
    public WebMessageDetail.Service getTestService() {
        AS4Service testService =
                connectorTestService.getTestService(DomibusConnectorBusinessDomain.getDefaultMessageLaneId());
    ...
    public WebMessageDetail.Action getTestAction() {
        AS4Action testAction =
                connectorTestService.getTestAction(DomibusConnectorBusinessDomain.getDefaultMessageLaneId());
    ...
    public DomibusConnectorMessageType mapWebMessageToTransition(WebMessage webMsg) {
        ...
        webMsg.getFiles().stream()
              ...
              .forEach(a -> {
                  ...
                  DataHandler dh =
                          ConversionTools.convertByteArrayToDataHandler(
                                  bytes,
    ...
    private DomibusConnectorMessageContentType mapContent(WebMessage webMsg) {
        ...
        Optional<WebMessageFile> businessContent = webMsg.getFiles().stream()
                                                         .filter(f -> f.getFileType()
                                                                       .equals(WebMessageFileType.BUSINESS_CONTENT))
                                                         .findFirst();
        ...
        Optional<WebMessageFile> businessDocument = webMsg.getFiles().stream()
                                                          .filter(f -> f.getFileType()
                                                                        .equals(WebMessageFileType.BUSINESS_DOCUMENT))
                                                          .findFirst();
        ...
        if (businessDocument.isPresent()) {
            ...
            DataHandler dh =
                    ConversionTools.convertByteArrayToDataHandler(fileContent, MimeTypeEnum.PDF.getMimeTypeString());
