...
public class WsBackendServiceEndpointImpl implements DomibusConnectorBackendWebService {
            ...
            if (backendClientInfoByName.isPresent()) {
                List<DomibusConnectorTransportStep> pendingTransportsForLinkPartner =
                        transportStateService
                                .getPendingTransportsForLinkPartner(backendClientInfoByName.get().getLinkPartnerName());
                List<DomibusConnectorMessageType> collect =
                        pendingTransportsForLinkPartner.stream()
                                                       .map(DomibusConnectorTransportStep::getTransportedMessage)
                                                       // java9 should handle this
                                                       // better: Optional::streamOf
            ...
            try {
                ...
                if (backendClientInfoByName.isPresent()) {
                    ...
                    try (
                        ...
                        DomibusConnectorMessage msg = transformerService.transformTransitionToDomain(
                                submitMessageRequest,
                                domibusConnectorMessageId
                        );
                        msg.getMessageDetails()
                           .setConnectorBackendClientName(linkPartner.getLinkPartnerName().getLinkName());
        ...
        try {
            ...
            if (backendClientInfoByName.isPresent()) {
                ...
                List<String> pendingIds =
                        pendingTransportsForLinkPartner.stream().map(DomibusConnectorTransportStep::getTransportId)
            ...
            if (domibusConnectorTransportStep.isInPendingState()) {
                if (domibusConnectorTransportStep.getTransportedMessage().isPresent()) {
                    ...
                    throw new IllegalStateException(String.format(
                            "The message with transport id [%s] is not " +
                                    "readable" + " anymore!",
                            messageTransportId
                    ));
                }
                ...
                throw new IllegalArgumentException(String.format(
                        "The message with transport id [%s] is not in " +
                                "pending state!",
                        messageTransportId
                ));
            }
    ...
    public EmptyRequestType acknowledgeMessage(DomibusConnectorMessageResponseType ack) {
        TransportStateService.TransportId transportId =
                new TransportStateService.TransportId(ack.getResponseForMessageId());
    ...
    private void registerCleanupTransportInterceptor(List<DomibusConnectorTransportStep> pendingTransportsForLinkPartner) {
        MessageContext mContext = webServiceContext.getMessageContext();
        WrappedMessageContext wmc = (WrappedMessageContext) mContext;
        ProcessMessageAfterDeliveredToBackendInterceptor interceptor =
                new ProcessMessageAfterDeliveredToBackendInterceptor(
                        pendingTransportsForLinkPartner.stream()
    ...
    private Optional<DomibusConnectorLinkPartner> checkBackendClient() throws DomibusConnectorBackendDeliveryException {
        ...
        if (userPrincipal == null || certificateDn == null) {
            java.lang.String error = java.lang.String.format(
                    "checkBackendClient: Cannot handle request because userPrincipal is [%s] the userName is [%s]. "
    ...
    private class ProcessMessageAfterDownloaded extends AbstractPhaseInterceptor<Message> {
        ...
        public void handleMessage(Message message) throws Fault {
            txTemplate.executeWithoutResult((state) -> {
                LOGGER.trace(
                        "ProcessMessageAfterDownloaded: handleMessage: invoking backendSubmissionService" +
                                ".processMessageAfterDownloaded setting transport set to "
                                + TransportState.PENDING_DOWNLOADED
                );
                TransportStateService.DomibusConnectorTransportState transportState =
                        new TransportStateService.DomibusConnectorTransportState();
    ...
    private class ProcessMessageAfterDeliveredToBackendInterceptor extends AbstractPhaseInterceptor<Message> {
        ...
        @Override
        public void handleMessage(Message message) throws Fault {
            LOGGER.trace("ProcessMessageAfterDeliveredToBackendInterceptor: handleMessage: invoking " +
                                 "backendSubmissionService.processMessageAfterDeliveredToBackend");
