...
All this data is stored by the connector within the database. There is no purge or deletion mechanism in place. So the products or usecases which are using the connector should make sure that no sensitive data is used within these fields.
...
The payload data is handled by the large file handling module. Currently, the connector has 2 modules for that (for mor details see link:architecture/payload_handling.html[Payload Handling]) :
...
For more information about logging see: link:../configuration/logging.html#logging[Logging Configuration]
