...
@ConditionalOnProperty(
        prefix = UploadPModesOnFirstStartConfigurationProperties.PREFIX, name = "enabled", havingValue = "true"
)
    ...
    private void processUpload(UploadPModesOnFirstStartConfigurationProperties.PModeUpload pModeUpload) {
        ...
        if (!businessDomainName.equals(DomibusConnectorBusinessDomain.DEFAULT_LANE_NAME)) {
            throw new IllegalArgumentException(
                    "Currently there is no bussiness domain support for " +
        ...
        try {
            ...
            byte[] trustStoreBytes = StreamUtils.copyToByteArray(
                    ctx.getResource(pModeUpload.getTrustStore().getPath()).getInputStream()
            );

            DomibusConnectorKeystore domibusConnectorKeystore = webPModeService
                    .importConnectorstore(trustStoreBytes, pw, DomibusConnectorKeystore.KeystoreType.ofDbName(type)
                    );
