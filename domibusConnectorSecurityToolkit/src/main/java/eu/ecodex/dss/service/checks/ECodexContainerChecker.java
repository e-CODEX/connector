...
public class ECodexContainerChecker extends AbstractChecker<ECodexContainer> {
    ...
    private void checkTokenValidation(final CheckResult r, final Token token) {
        ...
        // further constraints of the verification object depend on the system type
        //    (see checkTokenSystemType())
    ...
    private void checkTokenSystemType(final CheckResult r, final Token token) {
        ...
        if (!isMissing(
            // the absence is checked and reported in checkTokenValidation()
    ...
    private void checkTokenSystemTypeAuthBased(
        final CheckResult r, final ValidationVerification verification) {
        ...
        if (auth != null) {
            if (isMissing(auth.getUsernameSynonym())) {
                detect(
                    r, false, Findings.TOKEN_VALIDATION_VERIFICATIONDATA_AUTHINFO_USERNAME_MISSING);
            }
            if (isMissing(auth.getIdentityProvider())) {
                detect(
                    r, false,
                    Findings.TOKEN_VALIDATION_VERIFICATIONDATA_AUTHINFO_IDENTITYPROVIDER_MISSING
                );
            }
    ...
    private void checkTokenSystemTypeSigBased(
        final CheckResult r, final ValidationVerification verification) {
         ...
         * only if the data was signed, the attributes are taken into account
         * (the main object exists)
         */
        if (!sig.isEmpty()) {
            for (Signature signature : sig) {
                ...
                if (atts == null && cert == null && time == null && sig.size() == 1) {
                    // No validation needed. Document is unsigned in the way it was up to
                    // Version 1.08.4
                } else {
                    if (isMissing(cert)) {
                        detect(
                            r, true,
                            Findings.TOKEN_VALIDATION_VERIFICATIONDATA_SIGDATA_CERTINFO_MISSING
                        );
                    } else {
                    ...
                    if (isMissing(atts)) {
                        detect(
                            r, true,
                            Findings.TOKEN_VALIDATION_VERIFICATIONDATA_SIGDATA_SIGINFO_MISSING
                        );
                    } else {
                        ...
                        if (isMissing(atts.getSignatureLevel())) {
                            detect(
                                r, false,
                                Findings.TOKEN_VALIDATION_VERIFICATIONDATA_SIGDATA_SIGINFO_LEVEL_MISSING
                            );
                        }
                    }
                    ...
                    if (isMissing(time)) {
                        detect(
                            r, true,
                            Findings.TOKEN_VALIDATION_VERIFICATIONDATA_SIGDATA_TIME_MISSING
                        );
                    }
                }
            }
    ...
    public interface Findings {
        ...
        String TOKEN_DOCUMENT_MISSING =
            "the eCodexContainer's token object must not have a null/empty document object";
        ...
        String TOKEN_VALIDATION_TECHNICAL_RESULT_TRUSTLEVEL_MISSING =
            "the eCodexContainer's token technical validationresult object must not have a null/empty trustlevel object";
        String TOKEN_VALIDATION_TECHNICAL_RESULT_COMMENT_MISSING =
            "the eCodexContainer's token technical validationresult object should not have a null/empty comment, if the trustlevel is not SUCCESSFUL";
        ...
        String TOKEN_VALIDATION_LEGAL_RESULT_TRUSTLEVEL_MISSING =
            "the eCodexContainer's token legal validationresult object must not have a null/empty trustlevel object";
        ...
        String TOKEN_ISSUER_MISSING =
            "the eCodexContainer's token object must not have a null/empty issuer object";
        ...
        String TOKEN_VALIDATION_VERIFICATIONDATA_AUTHINFO_MISSING =
            "the eCodexContainer's token object must not have a null/empty authenticationinformation object, if the system is AUTHENTICATION_BASED";
        String TOKEN_VALIDATION_VERIFICATIONDATA_AUTHINFO_USERNAME_MISSING =
            "the eCodexContainer's token authenticationinformation object should not have a null/empty usernamesynonym, if the system is AUTHENTICATION_BASED";
        String TOKEN_VALIDATION_VERIFICATIONDATA_AUTHINFO_IDENTITYPROVIDER_MISSING =
            "the eCodexContainer's token authenticationinformation object should not have a null/empty identityprovider, if the system is AUTHENTICATION_BASED";
        String TOKEN_VALIDATION_VERIFICATIONDATA_AUTHINFO_TIME_MISSING =
            "the eCodexContainer's token authenticationinformation object should not have a null/empty timeofauthentication, if the system is AUTHENTICATION_BASED";
        String TOKEN_VALIDATION_VERIFICATIONDATA_SIGDATA_MISSING =
            "the eCodexContainer's token object must not have a null/empty signature object, if the system is SIGNATURE_BASED";
        String TOKEN_VALIDATION_VERIFICATIONDATA_SIGDATA_CERTINFO_MISSING =
            "the eCodexContainer's token signature object must not have a null/empty signaturecertificate object, if the system is SIGNATURE_BASED";
        String TOKEN_VALIDATION_VERIFICATIONDATA_SIGDATA_CERTINFO_ISSUER_MISSING =
            "the eCodexContainer's token signaturecertificate object should not have a null/empty issuer object, if the system is SIGNATURE_BASED";
        String TOKEN_VALIDATION_VERIFICATIONDATA_SIGDATA_SIGINFO_MISSING =
            "the eCodexContainer's token signature object must not have a null/empty signatureattributes object, if the system is SIGNATURE_BASED";
        String TOKEN_VALIDATION_VERIFICATIONDATA_SIGDATA_SIGINFO_FORMAT_MISSING =
            "the eCodexContainer's token signatureattributes object should not have a null/empty signatureformat object, if the system is SIGNATURE_BASED";
        String TOKEN_VALIDATION_VERIFICATIONDATA_SIGDATA_SIGINFO_LEVEL_MISSING =
            "the eCodexContainer's token signatureattributes object should not have a null/empty signaturelevel object, if the system is SIGNATURE_BASED";
        String TOKEN_VALIDATION_VERIFICATIONDATA_SIGDATA_TIME_MISSING =
            "the eCodexContainer's token signature object must not have a null/empty signingTime object, if the system is SIGNATURE_BASED";
        String CONTAINER_ASIC_MISSING =
            "the eCodexContainer must not have a null/empty asic document";
        String CONTAINER_ASIC_DATA_MISSING =
            "the eCodexContainer must not have a null/empty asic data stream";
        String CONTAINER_BUSINESS_MISSING =
            "the eCodexContainer must not have a null/empty business document";
        String CONTAINER_BUSINESS_DATA_MISSING =
            "the eCodexContainer must not have a null/empty business data stream";
        String CONTAINER_SIGNATURE_DATA_MISSING =
            "the eCodexContainer must not have a null/empty detached signature data stream";
        String CONTAINER_TOKENPDF_MISSING =
            "the eCodexContainer must not have a null/empty token PDF document";
        String CONTAINER_TOKENPDF_DATA_MISSING =
            "the eCodexContainer must not have a null/empty token PDF data stream";
        String CONTAINER_TOKENXML_MISSING =
            "the eCodexContainer must not have a null/empty token XML document";
        String CONTAINER_TOKENXML_DATA_MISSING =
            "the eCodexContainer must not have a null/empty token XML data stream";
    }
}
