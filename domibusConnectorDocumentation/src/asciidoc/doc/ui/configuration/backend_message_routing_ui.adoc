...
Most of the content types rely on the PMode-Set data. So the values can be taken from there.

==== Values

Values are strings that are compared. They are delimited by "*'*" symbols.
...
|===
|route expression|explanation|matching message content | not matching message content
