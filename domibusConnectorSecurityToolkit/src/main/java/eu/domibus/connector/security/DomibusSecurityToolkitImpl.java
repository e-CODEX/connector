...
public class DomibusSecurityToolkitImpl implements DomibusConnectorSecurityToolkit {
    ...
    public DomibusConnectorMessage validateContainer(DomibusConnectorMessage message)
        ...
        receiveContainerContents(message);
        return message;
    }
    ...
    BusinessContent buildBusinessContent(@Nonnull DomibusConnectorMessage message) {
        ...
        for (DomibusConnectorMessageAttachment attachment : message.getMessageAttachments()) {
            ...
            DSSDocument dssInMemoryDoc =
                    createLargeFileBasedDssDocument(attachment.getAttachment(), attachment.getName(), mimeType);
    ...
    public DomibusConnectorMessage createContainer(@Nonnull DomibusConnectorMessage message) {
        ...
        try {
            ...
            if (results.isSuccessful()) {
                if (container != null) {
                    ...
                    if (asicDocument != null) {
                        LOGGER.trace(
                                "converting asicDocument [{}] to asic message attachment and appending it to message",
                                asicDocument
                        );
                        DomibusConnectorMessageAttachment asicAttachment =
                                convertDocumentToMessageAttachment(message, asicDocument, ASICS_CONTAINER_IDENTIFIER);
                    ...
                    if (tokenXML != null) {
                        LOGGER.trace(
                                "converting tokenXml {[{}] to message attachment and appending it to message",
                                tokenXML
                        );
                        ...
                        DomibusConnectorMessageAttachment tokenAttachment =
                                convertDocumentToMessageAttachment(message, tokenXML, TOKEN_XML_IDENTIFIER);
            ...
            throw new DomibusConnectorSecurityException("ECodex exception occurred while creating container", e);
        } catch (IOException ioe) {
            throw new DomibusConnectorSecurityException("IOException occurred while creating container", ioe);
        }
    }
     ...
     * @param message - the message to process
     */
    public void receiveContainerContents(DomibusConnectorMessage message) {
        ECodexContainerService containerService = eCodexContainerFactoryService.createECodexContainerService(message);
        if (message.getMessageAttachments() != null && !message.getMessageAttachments().isEmpty()) {
            ...
            try (
                ...
                if (results.isSuccessful()) {
                    if (container != null) {
                        LOGGER.trace(
                                "receiveContainerContents: check if businessContent contains detachedSignature [{}]",
                        ...
                        if (container.getBusinessContent().getDetachedSignature() != null) {
                            ...
                            if (!StringUtils.isEmpty(container.getBusinessContent().getDetachedSignature().getName())) {
                                detachedSignatureBuilder.setName(container.getBusinessContent().getDetachedSignature()
                                                                          .getName());
                                LOGGER.trace(
                            ...
                            try {
                                LOGGER.trace(
                                        ...
                                        container.getBusinessContent().getDetachedSignature().getMimeType()
                                                 .getMimeTypeString()
                                );

                                detachedSignatureBuilder
                                        .setMimeType(
                                ...
                                LOGGER.error(
                                        "receiveContainerContents: No DetachedSignatureMimeType could be resolved of " +
                                                "MimeType [{}], using default MimeType [{}]",
                                        container
                                                .getBusinessContent()
                                                .getDetachedSignature()
                                                .getMimeType()
                        ...
                        if (container.getBusinessDocument() != null) {
                            LOGGER.debug(
                                    "The business document received from the container is of Mime Type {}",
                                    container.getBusinessDocument().getMimeType().getMimeTypeString()
                            );
                            try {
                                LargeFileReference bigDataRef =
                                        this.bigDataPersistenceService.createDomibusConnectorBigDataReference(
                                ...
                                LOGGER.trace(
                                        "copying businessDocument input stream to bigDataReference output " + "Stream"
                                );
                                try (
                                ...
                                LOGGER.trace(
                                        "receiveContainerContents: check if MimeType.PDF [{}] equals to [{}]",
                                        MimeTypeEnum.PDF.getMimeTypeString(),
                                        container.getToken().getDocumentType()
                                );

                                if (MimeTypeEnum.PDF.getMimeTypeString()
                                ...
                                LOGGER.trace(
                                        "receiveContainerContents: check if MimeType.XML [{}] equals to [{}]",
                                ...
                                if (MimeTypeEnum.XML.getMimeTypeString().equals(container.getToken().getDocumentType())
                                    ...
                                    // .getAction().isDocumentRequired()
                                ) {
                                    LOGGER.trace(
                                            "receiveContainerContents: Writing byteContent into MessageContent" +
                                                    ".setXmlContent"
                                    );
                        ...
                        if (tokenPDF != null) {
                            try {
                                ...
                                DomibusConnectorMessageAttachment attachment =
                                        convertDocumentToMessageAttachment(message, tokenPDF, TOKEN_PDF_IDENTIFIER);
                                message.addAttachment(attachment);
                            } catch (IOException e) {
    ...
    private DomibusConnectorMessageAttachment convertDocumentToMessageAttachment(
            ...
            throws IOException {
        LOGGER.trace(
                "convertDocumentToMessageAttachment: called with message [{}], document [{}], identifier [{}]",
                message,
                document,
                identifier
        );
        ...
        LOGGER.trace(
                "attachment created with bigDataRef [{}], and identifier [{}], name [{}], mimeTypeString [{}]",
                bigDataRef,
                identifier,
                documentName,
                mimeTypeString
