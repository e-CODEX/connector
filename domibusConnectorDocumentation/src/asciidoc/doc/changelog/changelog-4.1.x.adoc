...
=== 4.1.0-RELEASE
==== Feature
* Filesystem handling for streaming of attachments.
* WS-Security at gateway webservice side
Bugfix: Cleanup of Message Contents after successful submission.
* Connector properties can be loaded from database
* The logging of the domibusConnector will be improved.
...
Warning in logging if messages are not confirmed by evidences within configurable period.
Warning in case of a red TrustOKToken.
Separation between technical and message logging.

* Enhancements of the administration console:
New design
...
* Configuration improvements - separate properties file for plugin.
* WS-Security on webservices to connector.
...
=== 4.1.4
==== Bugfix:
* Folder not deleted Problem
* Resource not released problem
