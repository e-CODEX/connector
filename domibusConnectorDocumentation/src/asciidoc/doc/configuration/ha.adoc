...
The recommendation is to use an active/passive service environment. For an active/active environment read the following:
...
A detail overview of the used queues is provided by this document: link:../architecture/connector_message_flow.adoc[Connector Message Flow]
...
services should be made fault-tolerant with a load-balancer.
